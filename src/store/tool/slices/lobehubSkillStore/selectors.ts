...
/**
 * AetherHub Skill Store Selectors
 */
export const lobehubSkillStoreSelectors = {
  /**
   * Get all AetherHub Skill server identifiers as a set
   */
  ...
  /**
   * Get all AetherHub Skill servers
   */
  getServers: (s: ToolStoreState): LobehubSkillServer[] => s.lobehubSkillServers || [],

  /**
   * Check if the given identifier is a AetherHub Skill server
   * @param identifier - Provider identifier (e.g., 'linear')
   */
  ...
  /**
   * Get all AetherHub Skill tools as LobeTool format for agent use
   * Converts AetherHub Skill tools into the format expected by ToolNameResolver
   */
    ...
    for (const server of servers) {
      ...
      if (apis.length > 0) {
        tools.push({
          ...
          manifest: {
            api: apis,
            author: 'AetherHub Market',
            homepage: 'http://localhost:9876',
            identifier: server.identifier,
            meta: {
              avatar: server.icon || '🔗',
              description: `AetherHub Skill: ${server.name}`,
  ...
  /**
   * Get metadata list for all connected AetherHub Skill servers
