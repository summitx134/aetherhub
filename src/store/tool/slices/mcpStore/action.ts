...
const buildCloudMcpManifest = (params: {
  ...
  // Get tools (MCP format) or api (LobeHub format) from data
  const mcpTools = data.tools;
  const lobeChatApi = data.api;

  // If MCP format tools, need to convert to LobeHub api format
  // MCP: { name, description, inputSchema }
  // LobeHub: { name, description, parameters }
  let apiArray: any[] = [];

  if (lobeChatApi) {
    // Already in LobeHub format, use directly
    apiArray = lobeChatApi;
    log('[Cloud MCP] Using existing LobeHub API format');
  } else if (mcpTools && Array.isArray(mcpTools)) {
    // Convert MCP tools format to LobeHub api format
    apiArray = mcpTools.map((tool: any) => ({
      description: tool.description || '',
      name: tool.name,
      parameters: tool.inputSchema || {},
    }));
    log('[Cloud MCP] Converted %d MCP tools to LobeHub API format', apiArray.length);
  } else {
