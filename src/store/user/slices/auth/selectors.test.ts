...
describe('userProfileSelectors', () => {
  describe('displayUserName', () => {
    ...
    it('should return email when signed in but username is not existed in UserStore', () => {
      const store: UserStore = {
        isSignedIn: true,
        user: { email: 'demo@aetherhub.local' },
      } as UserStore;

      expect(userProfileSelectors.displayUserName(store)).toBe('demo@aetherhub.local');
    });
  ...
  describe('email', () => {
    it('should return user email if exist', () => {
      ...
      expect(userProfileSelectors.email(store)).toBe('demo@aetherhub.local');
    });
