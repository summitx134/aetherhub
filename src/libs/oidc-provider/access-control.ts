 ...
 */
export const revokeOIDCArtifactsByUserId = async (db: AetherHubDatabase, userId: string) => {
 ...
 */
export const assertOIDCUserActive = async (db: AetherHubDatabase, userId: string) => {
  const [user] = await db
