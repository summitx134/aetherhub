...
class DiscoverService {
  ...
  safeInjectMPToken = async () => {
    // Mock mode (SPA without real backend): skip MP token injection entirely.
    // The injection path calls registerClientInMarketplace → fails/times out on mock backend
    // → SWR fetchers never reach the actual data query → skeleton screen forever.
