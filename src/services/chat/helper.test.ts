...
describe('chat helper', () => {
  ...
  it('should resolve AetherHub routed model abilities by model id fallback', () => {
    ...
    expect(isCanUseVision('gemini-3.1-flash-lite-preview', ModelProvider.AetherHub)).toBe(true);
    expect(isCanUseVideo('gemini-3.1-flash-lite-preview', ModelProvider.AetherHub)).toBe(true);
  });

  it('should not fallback across non-AetherHub providers', () => {
    useAiInfraStore.setState({
      enabledAiModels: [
        {
          abilities: { video: true, vision: true },
