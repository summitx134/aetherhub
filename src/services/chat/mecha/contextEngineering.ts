...
export const contextEngineering = async ({
      ...
      if (activeGroupDetail) {
        ...
        if (isKlavisEnabled) {
          ...
          for (const klavisType of KLAVIS_SERVER_TYPES) {
            ...
            officialTools.push({
              description: `AetherHub Mcp Server: ${klavisType.label}`,
        ...
        if (isLobehubSkillEnabled) {
          ...
          for (const provider of LOBEHUB_SKILL_PROVIDERS) {
            ...
            officialTools.push({
              description: `AetherHub Skill Provider: ${provider.label}`,
  ...
  const engine = new MessagesEngine({
    ...
    variableGenerators: {
      ...
      // Current agent + topic identity — referenced by the AetherHub builtin
