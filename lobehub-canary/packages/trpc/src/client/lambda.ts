            ...
            switch (status) {
              case 401: {
                if (isMarketApi) {
                  ...
                  // Don't trigger AetherHub logout for market auth issues
                  const { getUserStoreState } = await import('@/store/user/store');
                  // Without a AetherHub session a market.* 401 is not a Market auth
                  ...
                  // Non-market 401: handle as before (AetherHub session expired)
                  const now = Date.now();
