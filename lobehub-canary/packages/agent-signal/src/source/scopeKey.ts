...
/**
 * Bot-thread identity used to route source events that originate outside a AetherHub topic.
 */
...
export interface AgentSignalProducerScopeInput {
  ...
  taskId?: string;
  /** Optional AetherHub topic identifier. Takes precedence over task and bot-thread metadata. */
  topicId?: string;
...
/**
 * Topic identity used when an AgentSignal chain is scoped to one AetherHub topic.
 */
export interface AgentSignalTopicScopeKeyInput {
  /** Stable AetherHub topic identifier. */
  topicId: string;
}
