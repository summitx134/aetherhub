...
export class UserModel {
  ...
  private db: AetherHubDatabase;

  constructor(db: AetherHubDatabase, userId: string) {
    this.userId = userId;
    this.db = db;
  }
  ...
  // Static method
  static makeSureUserExist = async (db: AetherHubDatabase, userId: string) => {
    await db.insert(users).values({ id: userId }).onConflictDoNothing();
  };

  static createUser = async (db: AetherHubDatabase, params: NewUser) => {
  ...
  static deleteUser = async (db: AetherHubDatabase, id: string) => {
    return db.delete(users).where(eq(users.id, id));
  };

  static findById = async (db: AetherHubDatabase, id: string) => {
    return db.query.users.findFirst({ where: eq(users.id, id) });
  };

  static findByUsername = async (db: AetherHubDatabase, username: string) => {
  ...
  static findByEmail = async (db: AetherHubDatabase, email: string) => {
    return db.query.users.findFirst({ where: eq(users.email, email) });
  };

  static findByIds = async (db: AetherHubDatabase, ids: string[]) => {
