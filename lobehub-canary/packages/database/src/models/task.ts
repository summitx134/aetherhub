...
export class TaskModel {
  ...
  private readonly db: AetherHubDatabase;
  private readonly workspaceId?: string;

  constructor(db: AetherHubDatabase, userId: string, workspaceId?: string) {
    this.db = db;
    this.userId = userId;
  ...
  static async getScheduledTasks(db: AetherHubDatabase): Promise<TaskItem[]> {
    return db
      .select()
      .from(tasks)
      .where(
  ...
  static async findStuckTasks(db: AetherHubDatabase): Promise<TaskItem[]> {
    return db
      .select()
      .from(tasks)
      .where(
   ...
   */
  private async collectTaskSubtree(rootId: string, runner: AetherHubDatabase): Promise<TaskItem[]> {
  ...
  async transferTo(
    ...
    return this.db.transaction(async (trx) => {
      const scoped = new TaskModel(trx as AetherHubDatabase, this.userId, this.workspaceId);
      const subtree = await scoped.collectTaskSubtree(taskId, trx as AetherHubDatabase);
      ...
      for (const [idx, task] of subtree.entries()) {
        ...
        await (trx as AetherHubDatabase)
          .update(tasks)
          .set({
  ...
  async copyToWorkspace(
    ...
    return this.db.transaction(async (trx) => {
      ...
      let seq = await this.nextSeqIn(trx as AetherHubDatabase, targetWorkspaceId, targetUserId);
      ...
      while (queue.length > 0) {
        ...
        const identifier = `T-${seq}`;
        const inserted = (await (trx as AetherHubDatabase)
          .insert(tasks)
          .values({
