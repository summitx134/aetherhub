...
export const getTestDB = async (): Promise<AetherHubDatabase> => {
  // Server DB mode (node-postgres)
  if (isServerDBMode) {
    if (testServerDB) return testServerDB as unknown as AetherHubDatabase;
    ...
    return testServerDB as unknown as AetherHubDatabase;
  }

  // Client DB mode (PGlite)
  if (testClientDB) return testClientDB as unknown as AetherHubDatabase;
  ...
  return testClientDB as unknown as AetherHubDatabase;
};
