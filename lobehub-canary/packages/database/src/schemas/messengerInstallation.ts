 ...
 * tenant_id)` so a single AetherHub deployment can serve many workspaces of the
 * same Slack App without collisions.
 *
 * Distinct from `agent_bot_providers` (per-agent-per-user bot bindings): this
 * row represents the AetherHub-owned, Marketplace-distributed app installed
...
export const messengerInstallations = pgTable(
  'messenger_installations',
  {
    ...
    /** AetherHub user who clicked "Connect Slack" — bound at OAuth callback. */
