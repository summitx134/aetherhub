...
/**
 * Maps a AetherHub user to a single IM account per platform (e.g. one Telegram
 * account ↔ one AetherHub user). The active agent for that IM session is
...
export const messengerAccountLinks = pgTable(
  ...
  (t) => [
    // One IM account per (platform, tenant) binds to exactly one AetherHub user.
    // The tenant column lets the same Slack user id under two workspaces
    // bind to different AetherHub users — without it, the second workspace
    ...
    // One AetherHub user has at most one IM account per (platform, tenant) —
