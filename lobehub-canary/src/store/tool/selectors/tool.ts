...
const getRenderDisplayControl =
  ...
  (s: ToolStoreState): RenderDisplayControl => {
    ...
    // Fallback for packages that don't ship a AetherHub manifest (e.g. Claude Code —
 ...
 * 2. User-installed plugins (from s.installedPlugins) — exclude Klavis/AetherHub Skill/agent skill overlap
 * 3. Klavis MCP servers (connected) — description from KLAVIS_SERVER_TYPES
 * 4. AetherHub Skill servers (connected) — description from LOBEHUB_SKILL_PROVIDERS
 */
const availableToolsForDiscovery = (s: ToolStoreState): AvailableToolForDiscovery[] => {
  ...
  //    Exclude Klavis, AetherHub Skill, and agent skill entries (they are handled in dedicated sources)
  ...
  // 4. AetherHub Skill servers (connected only)
