import { builtinSkills, AetherHubIdentifier } from '@lobechat/builtin-skills';
import { renderPlaceholderTemplate } from '@lobechat/context-engine';
import { describe, expect, it } from 'vitest';

const AetherHubSkill = builtinSkills.find((s) => s.identifier === AetherHubIdentifier);
if (!AetherHubSkill) {
  throw new Error(`AetherHubSkill not found in builtinSkills (looking for "${AetherHubIdentifier}")`);
}
const lobeHubContent = AetherHubSkill.content;
 ...
 * the AetherHub builtin skill content references `{{agent_id}}`, `{{topic_id}}`,
 ...
 */
describe('AetherHub skill identity placeholders ()', () => {
