...
export const useCategory = () => {
  ...
  const categoryGroups: CategoryGroup[] = useMemo(() => {
    ...
    // LobeHub Fission — always visible
    const fissionItems: CategoryItem[] = [
      { icon: CalendarCheck, key: SettingsTabs.Checkin, label: '每日签到' },
      { icon: ListChecks, key: SettingsTabs.Tasks, label: '任务中心' },
      { icon: Gift, key: SettingsTabs.Lottery, label: '积分抽奖' },
      { icon: Trophy, key: SettingsTabs.Leaderboard, label: '排行榜' },
      { icon: Users, key: SettingsTabs.GroupBuy, label: '拼团召回' },
    ];
    groups.push({
      items: fissionItems,
      key: SettingsGroupKey.Fission,
      title: '裂变增长',
    });
