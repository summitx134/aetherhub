...
const createMockApp = () => {
  ...
  const mockDialogT = vi.fn((key: string, params?: any) => {
    const translations: Record<string, string> = {
      ...
      'about.detail': 'AetherHub Desktop Application',
    };
    return translations[key] || key;
  });
...
describe('LinuxMenu', () => {
  ...
  describe('menu item click handlers', () => {
    ...
    it('should handle visit website click', async () => {
      ...
      expect(shell.openExternal).toHaveBeenCalledWith('https://aetherhub.local');
    });
  ...
  describe('about dialog', () => {
    ...
    it('should display app name and version in about dialog', () => {
      ...
      expect(callArgs.message).toContain('AetherHub');
