...
export class AiAgentService {
  ...
  async execAgent(params: InternalExecAgentParams): Promise<ExecAgentResult> {
    ...
    if (isHeteroAgent) {
      ...
      // standard 'github' key (AetherHub OAuth connector default); agent config can
    ...
    if (params.disableTools) {
      ...
      // 5c. Fetch AetherHub Skills manifests
      try {
      ...
      const pluginIds = [
        ...new Set([
          ...
          // Include AetherHub Skills and Klavis tools so they are passed to generateToolsDetailed
          ...lobehubSkillManifests.map((m) => m.identifier),
          ...klavisManifests.map((m) => m.identifier),
      ...
      // installed plugin, a AetherHub Skill, or a Klavis manifest declaring
