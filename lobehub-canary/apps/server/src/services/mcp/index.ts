...
export class MCPService {
  ...
  async listTools(params: MCPClientParams): Promise<AetherHubPluginApi[]> {
    ...
    return retry(
      async (bail, attemptNumber) => {
        ...
        try {
          ...
          return result.map<AetherHubPluginApi>((item) => ({
  ...
  private transformMCPToolToLobeAPI = (data: McpTool[]) => {
    return data.map<AetherHubPluginApi>((item) => ({
      // Assuming identifier is the unique name/id
      description: item.description,
      name: item.name,
      parameters: item.inputSchema as ToolManifestSettings,
