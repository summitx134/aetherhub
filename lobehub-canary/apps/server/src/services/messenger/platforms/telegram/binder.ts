...
export class MessengerTelegramBinder implements MessengerPlatformBinder {
  ...
  async handleUnlinkedMessage(ctx: UnlinkedMessageContext): Promise<void> {
    ...
    // The verify-im button takes the user back into AetherHub for the auth /
    ...
    try {
      ...
      await api.sendMessage(
        ctx.chatId,
        'AetherHub is temporarily unavailable. Please try again in a moment.',
      );
      return;
    }
  ...
  async notifyLinkSuccess(params: {
    ...
    const api = new TelegramApi(config.botToken);
    const headline = '✅ Linked successfully! Your AetherHub account is now connected.';
