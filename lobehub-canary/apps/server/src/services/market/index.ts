...
const MARKET_BASE_URL = process.env.MARKET_BASE_URL || 'http://localhost:9876';
...
export class MarketService {
  ...
  /**
   * Search for skills in the AetherHub Market
   */
  ...
  /**
   * Execute a AetherHub Skill tool
  ...
  /**
   * Fetch AetherHub Skills manifests from Market API
      ...
      for (const connection of connections) {
        try {
          ...
          const manifest: LobeToolManifest = {
            ...
            meta: {
              avatar: icon || '🔗',
              description: `AetherHub Skill: ${providerLabel}`,
