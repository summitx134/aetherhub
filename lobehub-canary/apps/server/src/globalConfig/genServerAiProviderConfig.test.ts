...
describe('genServerAiProvidersConfig', () => {
  ...
  it('should allow AetherHub models to come from the async model bank loader', async () => {
    const result = await genServerAiProvidersConfig({});

    expect(result).toHaveProperty('aetherhub');
    expect(result.aetherhub).toHaveProperty('serverModelLists');
    expect(Array.isArray(result.aetherhub.serverModelLists)).toBe(true);
  });
});

describe('genServerAiProvidersConfig Error Handling', () => {
  it('should throw error when a non-AetherHub provider is not found in aiModels', async () => {
    vi.resetModules();
