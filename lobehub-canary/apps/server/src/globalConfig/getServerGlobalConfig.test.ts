...
describe('getServerGlobalConfig', () => {
  ...
  it('should only enable AetherHub by default in business feature mode', async () => {
    const providerConfig = await loadCapturedProviderConfig(true);

    expect(providerConfig[ModelProvider.AetherHub].enabled).toBe(true);
    ...
    for (const provider of Object.values(ModelProvider)) {
      if (provider === ModelProvider.AetherHub) continue;
  ...
  it('should keep upstream defaults outside business feature mode', async () => {
    ...
    expect(providerConfig[ModelProvider.AetherHub]).toBeUndefined();
