...
const buildMockFileAccessUrl = ({ id }: { id: string }) => `https://aetherhub.local/f/${id}`;
...
describe('fileRouter', () => {
  ...
  describe('createFile', () => {
    ...
    it('should return proxy URL format ${APP_URL}/f/:id', async () => {
      ...
      expect(result).toEqual({
        id: 'new-file-id',
        url: 'https://aetherhub.local/f/new-file-id',
      });
    });
  ...
  describe('findById', () => {
    ...
    it('should return proxy URL format ${APP_URL}/f/:id', async () => {
      ...
      expect(result.url).toBe('https://aetherhub.local/f/test-id');
    });
  });
  ...
  describe('getFiles', () => {
    ...
    it('should return proxy URL format ${APP_URL}/f/:id for each file', async () => {
      ...
      expect(result[0].url).toBe('https://aetherhub.local/f/file-1');
      expect(result[1].url).toBe('https://aetherhub.local/f/file-2');
    });
  });
  ...
  describe('getKnowledgeItems', () => {
    ...
    it('should return knowledge items with files and documents', async () => {
      ...
      expect(result.items[0]).toMatchObject({
        ...
        id: 'file-1',
        sourceType: 'file',
        url: 'https://aetherhub.local/f/file-1',
      });
