  ...
  call_llm: async (instruction, state) => {
    ...
    try {
      ...
      if (agentConfig) {
        ...
        //   renders cleanly).
        const aetherhubSkillAgentId = state.metadata?.agentId;
        const aetherhubSkillTopicId = state.metadata?.topicId;
        const aetherhubSkillAgentMeta = state.metadata?.agentConfig as
          | { description?: string | null; title?: string | null }
          | undefined;

        let aetherhubSkillTopicTitle = '';
        if (aetherhubSkillTopicId && ctx.serverDB && ctx.userId) {
          try {
            const topicModelForLobehub = new TopicModel(ctx.serverDB, ctx.userId, ctx.workspaceId);
            const topicRecord = await topicModelForLobehub.findById(aetherhubSkillTopicId);
            aetherhubSkillTopicTitle = topicRecord?.title ?? '';
          } catch (error) {
            log('Failed to load topic title for aetherhub skill placeholders: %O', error);
          }
        }

        const aetherhubSkillVariables: Record<string, string> = {
          agent_id: aetherhubSkillAgentId ?? '',
          agent_title: aetherhubSkillAgentMeta?.title ?? '',
          agent_description: aetherhubSkillAgentMeta?.description ?? '',
          topic_id: aetherhubSkillTopicId ?? '',
          topic_title: aetherhubSkillTopicTitle,
        };
        ...
        if (sandboxEnabled === 'true' && ctx.serverDB && ctx.userId && aetherhubSkillTopicId) {
          try {
            ...
            const uploadedFiles = await fileModel.findFilesToInitInSandbox(aetherhubSkillTopicId);
    ...
    try {
      try {
        ...
        if (hookResult?.isMocked) {
          ...
          // aetherhubSkill / klavis tools correctly (LLM responses don't carry source)
