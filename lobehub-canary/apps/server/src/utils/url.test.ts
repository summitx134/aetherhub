...
describe('getCanonicalUrl', () => {
  ...
  it('should return correct URL for production environment', async () => {
    ...
    expect(result).toBe('https://aetherhub.local/path/to/page');
    expect(urlJoin).toHaveBeenCalledWith('https://aetherhub.local', 'path', 'to', 'page');
  });
  ...
  it('should work correctly without additional path arguments', async () => {
    ...
    expect(result).toBe('https://aetherhub.local');
    expect(urlJoin).toHaveBeenCalledWith('https://aetherhub.local');
  });
});
