...
vi.mock('../settings', () => ({
  loadSettings: vi.fn().mockReturnValue({ serverUrl: 'https://app.aetherhub.local' }),
  resolveServerUrl: vi.fn(() =>
    (process.env.LOBEHUB_SERVER || 'https://app.aetherhub.local').replace(/\/$/, ''),
  ),
}));
...
describe('resolveToken', () => {
  ...
  describe('with explicit --token', () => {
    it('should return token and userId from JWT', async () => {
      ...
      expect(result).toEqual({
        serverUrl: 'https://app.aetherhub.local',
        token,
        tokenType: 'jwt',
        userId: 'user-123',
      });
  ...
  describe('with environment api key', () => {
    it('should return API key from environment', async () => {
      ...
      expect(getUserIdFromApiKey).toHaveBeenCalledWith('sk-lh-test', 'https://app.aetherhub.local');
      expect(result).toEqual({
        serverUrl: 'https://app.aetherhub.local',
        token: 'sk-lh-test',
        tokenType: 'apiKey',
