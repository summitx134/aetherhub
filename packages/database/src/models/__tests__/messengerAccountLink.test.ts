...
describe('MessengerAccountLinkModel', () => {
  ...
  describe('findByPlatformUser (static)', () => {
    it('finds the right row when two users share the same Slack user id under different tenants', async () => {
      // Same Slack user id — but in different workspaces, bound to different AetherHub users.
      // (Could happen if two AetherHub accounts both happen to be `U_SHARED` in different workspaces.)
