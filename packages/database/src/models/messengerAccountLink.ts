 ...
 * different AetherHub user. Callers (e.g. the messenger router) should surface
...
export class MessengerAccountLinkConflictError extends Error {
  ...
  constructor(existingUserId: string, message?: string) {
    super(message ?? 'IM identity is already linked to another AetherHub user');
...
/**
 * Thrown when the same AetherHub user already has a different IM identity bound
...
export class MessengerAccountLinkModel {
   ...
   * AetherHub user; we throw `MessengerAccountLinkConflictError` instead.
   *
