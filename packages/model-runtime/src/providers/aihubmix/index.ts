...
/**
 * Maps AiHubMix `types` field values to AetherHub AiModelType.
 ...
 * AetherHub's AiModelType and are filtered out before model list processing to
...
/**
 * AiHubMix `types` values that have no corresponding AetherHub AiModelType.
...
/**
 * Map AiHubMix full-catalog API response fields to AetherHub model card fields.
...
const mapAiHubMixModel = (m: any): { [key: string]: any; id: string } => {
  ...
  return {
    ...
    // `structured_outputs` has no corresponding AetherHub model card field and is intentionally omitted.
    ...(featureSet && {
