...
export const createAsyncRouteContext = async (request: NextRequest): Promise<AsyncContext> => {
  ...
  log('AetherHub auth header present: %s', !!lobeChatAuthorization);
  ...
  if (!lobeChatAuthorization) {
    log('No AetherHub authorization header found');
    throw new Error('No AetherHub authorization header found');
  }
  ...
  try {
    ...
    log('Decrypting AetherHub authorization');
