...
export interface BillboardItem {
   ...
   * Keys use AetherHub locale codes (e.g. `zh-CN`, `en-US`, `ja-JP`).
   */
...
export interface BillboardSet {
  /** ISO timestamp — end of the display time window; AetherHub stops showing after this time */
  endAt: string;
  /**
  ...
  /** Unique identifier */
  slug: string;
  /** ISO timestamp — start of the display time window; AetherHub does not show before this time */
  startAt: string;
...
export interface EdgeConfigData extends BusinessEdgeConfigData {
   ...
   * dev deployments point to the dev store, prod deployments point to the prod store, transparent to the AetherHub side.
   */
