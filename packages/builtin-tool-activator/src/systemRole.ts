...
- User mentions: "SKILL.md", "AetherHub Skills", "skill store", "install skill", "search skill"
- User provides a GitHub link to install a skill (e.g., github.com/xxx/xxx containing SKILL.md)
- User mentions installing from AetherHub marketplace
- User provides AetherHub skill URLs like: \`https://aetherhub.local/skills/{identifier}/skill.md\` → extract identifier and use \`importFromMarket\`
- User provides instructions like: "curl https://aetherhub.local/skills/..." → extract identifier from URL, use \`importFromMarket\`
...
2. **For AetherHub skill URLs** (e.g., \`https://aetherhub.local/skills/{identifier}/skill.md\`):
   ...
   - Example: \`aetherhub.local/skills/openclaw-openclaw-github/skill.md\` → identifier is \`openclaw-openclaw-github\`
...
- For \`aetherhub.local/skills/xxx/skill.md\` URLs, ALWAYS extract the identifier and use \`importFromMarket\`, NOT \`importSkill\`
- \`importSkill\` is only for GitHub repository URLs or ZIP packages, not for aetherhub.local skill URLs
</skill_store_discovery>
...
4. If credential doesn't exist:
   - For AetherHub OAuth services (GitHub, Linear, Microsoft, Notion, Twitter) → use \`initiateOAuthConnect\`
...
- **SKILL-FIRST: Any mention of skills, SKILL.md, GitHub skill links, or AetherHub marketplace → activate \`lobe-skill-store\` FIRST, no exceptions.**
