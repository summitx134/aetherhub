...
export class CredsExecutionRuntime {
  ...
  async connectKlavisService(
    ...
    return {
      content: `To connect ${serverType.label}, please use the AetherHub app UI to initiate the Klavis OAuth flow. Server-side execution cannot open OAuth popups. Go to Settings or the onboarding page to connect ${serverType.label}.`,
      state: {
        connected: false,
        identifier: service,
  ...
  async initiateOAuthConnect(
    ...
    try {
      ...
      const appUrl = (process.env.APP_URL || 'https://app.aetherhub.local').replace(/\/+$/, '');
