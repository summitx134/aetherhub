...
describe('buildFilenameKeywordExpression', () => {
  ...
  it('splits whitespace-separated keywords into AND-ed substring terms', () => {
    // Critical fix: a free-form keyword string from the LLM (e.g. "AetherHub
    ...
    expect(buildFilenameKeywordExpression('AetherHub Financial Statement')).toBe(
      '(kMDItemFSName == "*AetherHub*"cd && kMDItemFSName == "*Financial*"cd && kMDItemFSName == "*Statement*"cd)',
    );
  });
