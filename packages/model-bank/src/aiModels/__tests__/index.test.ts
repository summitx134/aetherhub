...
describe('loadModels', () => {
  ...
  it('overrides provider models with injected async loaders', async () => {
    ...
    const models = await loadModels({
      providerLoaders: {
        [ModelProvider.AetherHub]: loader,
      },
    });
    ...
    expect(models).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          enabled: true,
          id: 'injected-lobehub-model',
          providerId: ModelProvider.AetherHub,
          source: 'builtin',
          type: 'chat',
        }),
      ]),
  ...
  it('ignores undefined provider loaders', async () => {
    await expect(
      loadModels({
        providerLoaders: {
          [ModelProvider.AetherHub]: undefined,
        },
      }),
...
describe('knowledgeCutoff backfill', () => {
  ...
  it('keeps an explicit knowledgeCutoff over the map value', async () => {
    ...
    const models = await loadModels({
      providerLoaders: { [ModelProvider.AetherHub]: loader },
    });

    const lobehubModels = models.filter((m) => m.providerId === ModelProvider.AetherHub);
