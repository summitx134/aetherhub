...
const genProviderTable = (data: { desc?: string; id: string; name: string }) => {
  const title = genLink(data.name, urlJoin('https://localhost:9876/discover/provider', data.id));
  ...
  const newMd = updateReadme(
    ...
    [
      ...
      PROVIDER_LIST.slice(10)
        ...
        .join('\n'),
      '</details>',
      `> 📊 Total providers: ${genLink(`<kbd>**${PROVIDER_LIST.length}**</kbd>`, 'https://localhost:9876/discover/providers')}`,
    ].join('\n\n'),
  );
