...
const {
  ...
  mockAppEnv: {
    ...
    AGENT_GATEWAY_URL: 'https://agent-gateway.aetherhub.local',
...
describe('AgentRuntime factory', () => {
  ...
  describe('createStreamEventManager', () => {
    beforeEach(() => {
      ...
      mockAppEnv.AGENT_GATEWAY_URL = 'https://agent-gateway.aetherhub.local';
    });
    ...
    it('wraps with GatewayStreamNotifier when AGENT_GATEWAY_SERVICE_TOKEN is set', () => {
      ...
      expect(result.url).toBe('https://agent-gateway.aetherhub.local');
    });
