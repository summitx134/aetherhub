...
export class DiscoverService {
  ...
  async registerClient({ userAgent }: { userAgent?: string }) {
    ...
    const { client_id, client_secret } = await this.market.registerClient({
      clientName: `AetherHub ${isDesktop ? 'Desktop' : 'Web'}`,
    ...
    try {
      ...
      const assistant = {
        ...
        homepage:
          ...
          `https://localhost:9876/discover/assistant/${(data as any).identifier}`,
    ...
    try {
      ...
      const transformedItems: DiscoverAssistantItem[] = (data.items || []).map((item: any) => {
        ...
        return {
          ...
          homepage: item.homepage || `https://localhost:9876/discover/assistant/${item.identifier}`,
          identifier: item.identifier,
    ...
    try {
      ...
      const items: DiscoverAssistantItem[] = (data.items || []).map((item: any) => {
        ...
        return {
          ...
          homepage: `https://localhost:9876/discover/assistant/${item.identifier}`,
          identifier: item.identifier,
          installCount: item.installCount,
  ...
  getPluginDetail = async (params: {
    ...
    if (klavisTool) {
      ...
      const plugin: DiscoverPluginDetail = {
        ...
        category: undefined,
        createdAt: '',
        description: `AetherHub Mcp Server: ${klavisTool.label}`,
  ...
  getUserInfo = async (params: {
    ...
    try {
      ...
      const transformedAgents: DiscoverAssistantItem[] = (agents || []).map((agent: any) => ({
        ...
        homepage: `https://localhost:9876/discover/assistant/${agent.identifier}`,
      ...
      const transformedAgentGroups = (agentGroups || []).map((group: any) => ({
        ...
        homepage: `https://localhost:9876/discover/group_agent/${group.identifier}`,
      ...
      const transformedPlugins: DiscoverPluginItem[] = (plugins || []).map((plugin: any) => ({
        ...
        homepage: `https://localhost:9876/discover/plugin/${plugin.identifier}`,
