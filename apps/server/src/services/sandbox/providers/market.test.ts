...
describe('MarketSandboxProvider', () => {
  ...
  describe('redactSandboxParams', () => {
    it('redacts auth env assignments from command logs without changing other params', () => {
      const params = {
        command:
          'LOBEHUB_JWT=mock-jwt LOBEHUB_SERVER=https://app.aetherhub.local npx -y @lobehub/cli topic list && GITHUB_TOKEN="ghp_token" gh repo view',
        timeout: 1000,
      };

      expect(redactSandboxParams(params)).toEqual({
        command:
          'LOBEHUB_JWT=[redacted] LOBEHUB_SERVER=https://app.aetherhub.local npx -y @lobehub/cli topic list && GITHUB_TOKEN=[redacted] gh repo view',
        timeout: 1000,
      });
    });
