  ...
  it('collects src from images and fileUrl from files, recursively', () => {
    const json = {
      root: {
        children: [
          {
            children: [image('https://app.aetherhub.local/f/file_nested')],
            type: 'paragraph',
          },
          file('https://app.aetherhub.local/f/file_pdf', 'doc.pdf'),
        ],
      },
    };
    expect(collectAttachmentUrlsFromEditorData(json)).toEqual([
      'https://app.aetherhub.local/f/file_nested',
      'https://app.aetherhub.local/f/file_pdf',
    ]);
  });
