 ...
 * which AetherHub context a conversation runs in is derived from the *active
...
export const messengerRouter = router({
   ...
   * a AetherHub account — the page uses it to warn the user before they create
  ...
  peekLinkToken: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return {
        // Set when the IM identity is already linked to some AetherHub account.
  ...
  confirmLink: messengerProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // already bound to a different AetherHub account. The DB unique index
      ...
      try {
        ...
        // Race backstop: the IM identity got bound to another AetherHub user
  ...
  /**
   * List the Slack workspaces this AetherHub user has installed the bot into.
  ...
  uninstallInstallation: messengerWriteProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // it. Workspace admins who installed via a different AetherHub account
