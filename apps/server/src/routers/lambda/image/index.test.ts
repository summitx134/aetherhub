...
const {
  ...
  mockIsAetherHubModelAvailable: vi.fn(),
...
describe('imageRouter', () => {
  ...
  beforeEach(() => {
    ...
    mockIsAetherHubModelAvailable.mockResolvedValue(true);
  ...
  describe('createImage', () => {
    ...
    it('should validate mapped model id before rejecting deprecated lobehub image models', async () => {
      ...
      expect(mockIsAetherHubModelAvailable).toHaveBeenCalledWith('gpt-image-1', 'image', {
        getUserEmail: expect.any(Function),
      });
      const availabilityOptions = mockIsAetherHubModelAvailable.mock.calls.at(-1)?.[2];
    ...
    it('should reject unavailable lobehub image models before creating async tasks', async () => {
      mockIsAetherHubModelAvailable.mockResolvedValue(false);
      ...
      await expect(caller.createImage(input)).rejects.toMatchObject({
        code: 'BAD_REQUEST',
        message: 'AetherHubModelDeprecated',
      });
