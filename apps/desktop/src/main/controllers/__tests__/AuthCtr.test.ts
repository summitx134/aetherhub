...
vi.mock('@/const/env', () => ({
  OFFICIAL_CLOUD_SERVER: 'https://aetherhub-cloud.com',
  isMac: false,
  isWindows: false,
  isLinux: false,
  isDev: false,
...
const mockRemoteServerConfigCtr = {
  ...
  getRemoteServerUrl: vi.fn().mockImplementation(async (config?: DataSyncConfig) => {
    ...
    return 'https://aetherhub-cloud.com'; // OFFICIAL_CLOUD_SERVER
  }),
    ...
    describe('requestAuthorization', () => {
      it('should generate PKCE parameters and open authorization URL', async () => {
        ...
        expect(shell.openExternal).toHaveBeenCalledWith(
          expect.stringContaining('https://aetherhub-cloud.com/oidc/auth'),
        );

        // Verify URL contains required parameters
        const authUrl = vi.mocked(shell.openExternal).mock.calls[0][0];
        expect(authUrl).toContain('client_id=aetherhub-desktop');
  ...
  describe('Proactive Token Refresh', () => {
    beforeEach(() => {
      vi.mocked(mockRemoteServerConfigCtr.getRemoteServerConfig).mockResolvedValue({
        active: true,
        remoteServerUrl: 'https://aetherhub-cloud.com',
        storageMode: 'cloud',
      });
