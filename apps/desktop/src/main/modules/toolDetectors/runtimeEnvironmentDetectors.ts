...
/**
 * AetherHub CLI detector
 * Tries lobehub, lobe, lh in order; validates via --help output containing "AetherHub"
 */
export const lobehubDetector: IToolDetector = {
  description: 'AetherHub CLI - manage and connect to AetherHub services',
  async detect(): Promise<ToolStatus> {
    ...
    for (const cmd of commands) {
      try {
        ...
        // Validate it's actually AetherHub CLI by checking help output
        const { stdout: helpOut } = await execPromise(`${cmd} --help`, { timeout: 3000 });
        if (!helpOut.includes('AetherHub')) continue;
